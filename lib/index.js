...
const isSnippet = false
const defaultOpt = {
    findNameDicAndSpace: !isSnippet,
    removeRedundantLogic: !isSnippet
}
const path = require('path')
const {isSameExpression, extend, getMemberExpStr} = require('./utils.js')
const removeRedudantLogic = require('./removeRedudantLogic')
const DecodeSwitchUtil = require('./decodeSwitch')

class JsTransform {
    constructor(opt = {}) {
        let sourceCode = this.sourceCode = fs.readFileSync(path.resolve(__dirname, `../source/${isSnippet ? "snippet" : "source"}.js`),{encoding:"utf-8"})
    ...
    init() {
        ...
        if (this.findNameDicAndSpace) {
            ...
            // console.log(this.getNameDic(1205))
            // return
            traverser.default(this.ast, {
                enter(path) {
       ...
       function replaceCall2Str(path, baseNames = [config.baseFuncName]) {
        ...
        //   }
          if (~self.getNameDicFnArr.indexOf(callee.property.name) && ~baseNames.indexOf(callee.object.name)) {
            ...
            str = unescape(str)
            // console.log(babelTypes.stringLiteral(str))
        ...
        traverser.default(this.ast, {
            VariableDeclarator(path) {
                ...
                if (node.init&&node.init.name ===config.baseFuncName) {
                    ...
                    functionPath.path.traverse({
                        BlockStatement(path) {
                            useDecodeSwitch(path, [config.baseFuncName, varBaseFnName])
                        }
                    })
                    path.skip()
                }  
            ...
            BlockStatement(path) {
                useDecodeSwitch(path)
            }
        })
        function useDecodeSwitch(path, baseNames = [config.baseFuncName]) {
           new DecodeSwitchUtil({path, baseNames, getNameSpace: self.getNameSpace})
        }
    }
    decode16Str(path) {
        let node = path.node
    ...
    generateCode(node) {
        let {code} = generator.default(node,{jsonCompatibleStrings:true})
        fs.writeFileSync(path.resolve(__dirname, '../result/result.js'), code, {encoding: "utf-8"})
    }
